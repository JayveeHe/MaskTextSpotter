...
class SequencePredictor(nn.Module):
    def __init__(self, cfg, dim_in):
        ...
        if cfg.SEQUENCE.TWO_CONV:
            ...
            self.seq_encoder = nn.Sequential(nn.Conv2d(dim_in, 256, 3, padding=1), nn.ReLU(inplace=True),
                                             nn.MaxPool2d(2, stride=2, ceil_mode=True))
        self.seq_decoder = BahdanauAttnDecoderRNN(256, cfg.SEQUENCE.NUM_CHAR, cfg.SEQUENCE.NUM_CHAR, n_layers=1,
                                                  dropout_p=0.1)
    ...
    def forward(self, x, decoder_targets=None, word_targets=None, use_beam_search=False):
        ...
        # x_t = self.quant(x_t)
        # y_t = self.quant(y_t)
        x_onehot_embedding = self.x_onehot(x_t).transpose(0, 2).transpose(1, 2).repeat(seq_decoder_input.size(0), 1, 1,
                                                                                       1)
        y_onehot_embedding = self.y_onehot(y_t).transpose(0, 2).transpose(1, 2).repeat(seq_decoder_input.size(0), 1, 1,
                                                                                       1)
        x_onehot_embedding = self.quant(x_onehot_embedding)
        y_onehot_embedding = self.quant(y_onehot_embedding)
        seq_decoder_input_loc = torch.cat([seq_decoder_input, x_onehot_embedding, y_onehot_embedding], 1)
        seq_decoder_input_reshape = seq_decoder_input_loc.view(seq_decoder_input_loc.size(0),
                                                               seq_decoder_input_loc.size(1), -1).transpose(0,
                                                                                                            2).transpose(
            1, 2)
        seq_decoder_input_reshape = self.dequant(seq_decoder_input_reshape)
        if self.training:
            ...
            if use_beam_search:
                for batch_index in range(seq_decoder_input_reshape.size(1)):
                    ...
                    top_seqs = self.beam_search(seq_decoder_input_reshape[:, batch_index:batch_index + 1, :],
                                                decoder_hidden, beam_size=6, max_len=self.cfg.SEQUENCE.MAX_LENGTH)
                    top_seq = top_seqs[0]
...
class Attn(nn.Module):
    ...
    def forward(self, hidden, encoder_outputs):
        ...
        encoder_outputs = self.quant(encoder_outputs)
        H = self.quant(H)
    ...
    def score(self, hidden, encoder_outputs):
        energy = torch.tanh(
            self.attn(torch.cat([hidden, encoder_outputs], 2)))  # (B, H*W, 2*hidden_size+H+W)->(B, H*W, hidden_size)
        energy = energy.transpose(2, 1)  # (B, hidden_size, H*W)
        energy = self.dequant(energy)
...
class BahdanauAttnDecoderRNN(nn.Module):
    ...
    def forward(self, word_input, last_hidden, encoder_outputs):
        ...
        if not self.training:
            ...
            tmp_out = self.out(self.quant(hidden))
            output = F.softmax(self.dequant(tmp_out), dim=1)
        else:
            # output = F.log_softmax(self.out(hidden), dim=1)
            tmp_out = self.out(self.quant(hidden))
            output = F.log_softmax(self.dequant(tmp_out), dim=1)
